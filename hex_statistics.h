...
private:
  struct EngineStats
  {
    ...
    void print() const
    {
      ...
      auto total_time =
          std::accumulate(move_times.begin(), move_times.end(), std::chrono::microseconds(0));
      auto avg_time = total_time / move_times.size();

      auto sec{std::chrono::duration_cast<std::chrono::seconds>(avg_time)};
      auto us{avg_time - std::chrono::duration_cast<std::chrono::microseconds>(sec)};

      double win_rate =
          (wins + losses > 0) ? (static_cast<double>(wins) / (wins + losses)) * 100.0 : 0.0;
...
public:
  ...
  void record_move_time(const std::string& engine_name, std::chrono::microseconds duration)
  {
  ...
  void export_to_csv(const std::string& filename) const
  {
    ...
    file << "EngineType,StartingPosition,Wins,Losses,WinRate,LossRate,TotalMoves,"
    ...
    for (const auto& [full_name, stats] : engines)
    {
      ...
      {
        auto total_time = std::accumulate(stats.move_times.begin(), stats.move_times.end(),
                                          std::chrono::microseconds(0));
        avg_time = total_time / stats.move_times.size();
      }

      file << engine_type << "," << starting_position << "," << stats.wins << "," << stats.losses
           << "," << std::fixed << std::setprecision(2) << win_rate << "," << std::fixed
           << std::setprecision(2) << loss_rate << "," << stats.total_moves << ","
           << avg_time.count() << "\n";
    }
  }
};

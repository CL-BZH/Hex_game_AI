#ifndef HEX_MACHINE_BF_H
#define HEX_MACHINE_BF_H
...
{
  HexMachineBF(uint board_size, uint threads = 1)
  ...
  {
    std::vector<std::array<uint, 2>> available_positions{};
    ...
    // store the available positions in the vector Positions_quality and the
    // threads for the bf_task will compute the quality of each position.
    results.Positions_quality.clear();
    for (auto pos : available_positions)
    {
      uint board_row = pos[0];
      uint board_column = pos[1];
      ...
      position.node_id =
          (board_row + 1) * (board_size + 2) + (board_column + 1);
...
private:
  std::atomic<uint> bf_task_call_count;
   ...
   *       So, at the end, when all possible combinations would have been
   * evaluted each available cell on the board will have a certain quality and
   * the one with the highest score will be selected.
   */
  void bf_task(const std::vector<std::array<uint, 2>>& selected_positions,
               HexBoard& board, uint player_id)
  {
    ...
    Path shortest_path;
    bool winning_board{
        board_copy->has_won(player_id, &shortest_path, &quality)};
    ...
    uint missing_node_id;
    bool game_over{true};

    if (!winning_board)
    {
      opponent_winning_board =
          board_copy->has_won(opponent_id, nullptr, &opponent_quality);
      game_over = false;
    }
    else
    {
      ...
      uint missing_position{0};
      Node missing_node;
      uint node_value;
      for (auto node : shortest_path.route)
      {
        node_value = static_cast<uint>(board.get_node_value(node.id));
    ...
    uint missing_cell_id;
    if (game_over)
    {
      // Get the cell id on the board
      uint row{
          static_cast<uint>(std::floor(missing_node_id / (board_size + 2)))};
      uint column{missing_node_id - row * (board_size + 2)};
    ...
    for (size_t i{0}; i < results.Positions_quality.size(); ++i)
    {
      ...
      {
        uint board_row{selected_positions[j][0]};
        uint board_column{selected_positions[j][1]};
        uint cell_id{board_row * board_size + board_column};
        ...
        if (results.Positions_quality[i].cell_id == cell_id)
        {
          ...
          {
            results.lock();
            // When lost (i.e. quality==0 and opponent_quality>0) then the
            // quality of the selected cell is decreased by the quality of the
            // opponent
   ...
   * combinations for each thread is done in the recursive function
   * n_choose_k(). For each combination that it computes, n_choose_k() calls the
   * function bf_task() that evaluates the quality of each selected positions on
   * the board for that particular combination. At the end, each available
   * positions will have a quality score (sum of all quality score for each
   * combination) and the best position to move on will be selected.
   */
  void spawn_threads(std::vector<std::array<uint, 2>>& available_positions,
                     uint machine_player_id, HexBoard& board)
  {
    ...
    uint total_threads{std::min(threads, k)};
    uint t{0};
    for (; t < total_threads - 1; ++t)
    {
      ...
      spawned_thread.push_back(
          std::thread(&HexMachineBF::choose, this, offset, k - 1, indexes,
    ...
    chosen.clear();
    offset = t;
    spawned_thread.push_back(std::thread(
        &HexMachineBF::choose, this, offset, k, indexes, chosen,
        &HexMachineBF::bf_task, available_positions, machine_player_id, board));
   ...
   */
  void choose(size_t offset, size_t k, std::vector<uint> all_indexes,
              std::vector<uint> chosen,
              void (HexMachineBF::*f)(const std::vector<std::array<uint, 2>>&,
                                      HexBoard&, uint),
              std::vector<std::array<uint, 2>> positions, uint player_id,
              HexBoard board)
  {
   ...
   */
  void n_choose_k(size_t offset, size_t k, std::vector<uint>& all_indexes,
                  std::vector<uint>& chosen,
                  void (HexMachineBF::*f)(
                      const std::vector<std::array<uint, 2>>&, HexBoard&, uint),
                  std::vector<std::array<uint, 2>>& positions, uint player_id,
                  HexBoard& board)
  {
    if (k == 0)
    {
      ...
      std::vector<std::array<uint, 2>> selected_positions;
